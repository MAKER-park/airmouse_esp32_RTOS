...
int mpu_lp_accel_mode(unsigned char rate)
{
    ...
    if (!rate) {
        ...
        tmp[0] = 0;
        tmp[1] = BIT_STBY_XYZG;
        if (i2c_write(st.reg->pwr_mgmt_1, tmp, 2))
            return -1;
        st.chip_cfg.lp_accel_mode = 0;
        return 0;
    }
...
int mpu_get_gyro_reg(short *data, unsigned long *timestamp)
{
    ...
    if (i2c_read(st.reg->raw_gyro, tmp, 6))
        return -1;
...
int mpu_get_accel_reg(short *data, unsigned long *timestamp)
{
    ...
    if (i2c_read(st.reg->raw_accel, tmp, 6))
        return -1;
    data[0] = (tmp[0] << 8) | tmp[1];
    data[1] = (tmp[2] << 8) | tmp[3];
    data[2] = (tmp[4] << 8) | tmp[5];
...
int mpu_reset_fifo(void)
{
    ...
    data = 0;
    if (i2c_write(st.reg->int_enable, &data, 1))
        return -1;
    if (i2c_write(st.reg->fifo_en, &data, 1))
        return -1;
    if (i2c_write(st.reg->user_ctrl, &data, 1))
        return -1;
    ...
    if (st.chip_cfg.dmp_on) {
        ...
        if (i2c_write(st.reg->int_enable, &data, 1))
            return -1;
        if (i2c_write(st.reg->fifo_en, &st.chip_cfg.fifo_enable, 1))
            return -1;
    }
    return 0;
}
...
int mpu_set_gyro_fsr(unsigned short fsr)
{
    ...
    if (st.chip_cfg.gyro_fsr == (data >> 3))
        return 0;
    if (i2c_write(st.reg->gyro_cfg, &data, 1))
        return -1;
...
int mpu_set_accel_fsr(unsigned char fsr)
{
    ...
    if (st.chip_cfg.accel_fsr == (data >> 3))
        return 0;
    if (i2c_write(st.reg->accel_cfg, &data, 1))
        return -1;
...
int mpu_set_lpf(unsigned short lpf)
{
    ...
    if (st.chip_cfg.lpf == data)
        return 0;
    if (i2c_write(st.reg->lpf, &data, 1))
        return -1;
...
int mpu_set_sample_rate(unsigned short rate)
{
    ...
    if (st.chip_cfg.dmp_on)
        ...
        data = 1000 / rate - 1;
        if (i2c_write(st.reg->rate_div, &data, 1))
            return -1;
...
int mpu_set_sensors(unsigned char sensors)
{
    ...
    if (!(sensors & INV_XYZ_ACCEL))
        data |= BIT_STBY_XYZA;
    if (i2c_write(st.reg->pwr_mgmt_2, &data, 1)) {
        st.chip_cfg.sensors = 0;
        return -1;
    }
...
int mpu_get_int_status(short *status)
{
    ...
    if (!st.chip_cfg.sensors)
        return -1;
    if (i2c_read(st.reg->dmp_int_status, tmp, 2))
        return -1;
        ...
        unsigned char *sensors, unsigned char *more)
{
    ...
    if (i2c_read(st.reg->fifo_count_h, data, 2))
        return -1;
    ...
    if (i2c_read(st.reg->fifo_r_w, data, packet_size))
        return -1;
    ...
    unsigned char *more)
{
    ...
    if (!st.chip_cfg.dmp_on)
    {
//    	printf("mpu_read_fifo_stream 1\n");
    	return -1;
    }
    if (!st.chip_cfg.sensors)
    {
//    	printf("mpu_read_fifo_stream 2\n");
    	return -1;
    }

    if (i2c_read(st.reg->fifo_count_h, tmp, 2))
    {
//    	printf("mpu_read_fifo_stream 3\n");
    	return -1;
    }
    ...
    if (fifo_count < length) {
        more[0] = 0;
        {
//        	printf("mpu_read_fifo_stream 4\n");
        	return -1;
        }
    }
    if (fifo_count > (st.hw->max_fifo >> 1)) {
        /* FIFO is 50% full, better check overflow bit. */
        if (i2c_read(st.reg->int_status, tmp, 1))
            {
//        		printf("mpu_read_fifo_stream 5\n");
        		return -1;
            }
        if (tmp[0] & BIT_FIFO_OVERFLOW) {
            mpu_reset_fifo();
            {
//            	printf("mpu_read_fifo_stream 6\n");
            	return -2;
            }
        }
    }

    if (i2c_read(st.reg->fifo_r_w, data, length))
        {
//    		printf("mpu_read_fifo_stream 7\n");
    		return -1;
        }
...
int mpu_set_bypass(unsigned char bypass_on)
{
    ...
    if (bypass_on) {
        if (i2c_read(st.reg->user_ctrl, &tmp, 1))
            return -1;
        tmp &= ~BIT_AUX_IF_EN;
        if (i2c_write(st.reg->user_ctrl, &tmp, 1))
            return -1;
        ...
        if (i2c_write(st.reg->int_pin_cfg, &tmp, 1))
            return -1;
    } else {
...
static int get_st_biases(long *gyro, long *accel, unsigned char hw_test)
{
    ...
    data[0] = 0x01;
    data[1] = 0;
    if (i2c_write(st.reg->pwr_mgmt_1, data, 2))
        return -1;
    ...
    if (i2c_write(st.reg->pwr_mgmt_1, data, 1))
        return -1;
    if (i2c_write(st.reg->i2c_mst, data, 1))
        return -1;
    if (i2c_write(st.reg->user_ctrl, data, 1))
        return -1;
    ...
    if (i2c_write(st.reg->fifo_en, data, 1))
        return -1;
    vTaskDelay(test.wait_ms / portTICK_PERIOD_MS);
    data[0] = 0;
    if (i2c_write(st.reg->fifo_en, data, 1))
        return -1;
    ...
    for (ii = 0; ii < packet_count; ii++) {
        ...
        if (i2c_read(st.reg->fifo_r_w, data, MAX_PACKET_LENGTH))
            return -1;
        ...
        unsigned char *data)
{
    ...
    if (i2c_write(st.reg->bank_sel, tmp, 2))
        return -1;
    if (i2c_write(st.reg->mem_r_w, data, length))
        return -1;
    return 0;
}
        ...
        unsigned char *data)
{
    ...
    if (i2c_write(st.reg->bank_sel, tmp, 2))
        return -1;
    if (i2c_read(st.reg->mem_r_w, data, length))
        return -1;
    return 0;
}
...
#define LOAD_CHUNK  (16)
    ...
    if (i2c_write(st.reg->prgm_start_h, tmp, 2))
        return -1;
    ...
    unsigned char lpa_freq)
{
    ...
    if (lpa_freq) {
        ...
        data[0] = thresh_hw;
        data[1] = time;
        if (i2c_write(st.reg->motion_thr, data, 2))
            goto lp_int_restore;
...
//返回值:0,正常
//    其他,失败
uint8_t mpu_dmp_get_data(float *att, float* acc, float* gyr)
{
    int i = 0;
    ...
    // added
    for (i=0; i<3; i++){
        acc[i] = (float)(accel[i])/32768.0f*2.0f;
        gyr[i] = (float)(gyro[i])/32768.0f*2000.0f;
    }
	...
	**/
	if(sensors&INV_WXYZ_QUAT)
	{
		...
		q3 = quat[3] / q30;
		//计算得到俯仰角/横滚角/航向角
		att[0] = asin(-2 * q1 * q3 + 2 * q0* q2)* 57.3f;	// pitch
		att[1]  = atan2(2 * q2 * q3 + 2 * q0 * q1, -2 * q1 * q1 - 2 * q2* q2 + 1)* 57.3f;	// roll
		att[2]   = atan2(2*(q1*q2 + q0*q3),q0*q0+q1*q1-q2*q2-q3*q3) * 57.3f;	//yaw
	}else return 2;
	return 0;
}