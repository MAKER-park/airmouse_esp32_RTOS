...
void hid_mouse_task(int8_t x, int8_t y){
    ...
    for (uint8_t i = 0; i < CONFIG_BT_ACL_CONNECTIONS; i++) // mouse pointer move
    {
        if (active_hid_conn_ids[i] != -1)
            esp_hidd_send_mouse_value(active_hid_conn_ids[i], 0, x*-1, y, 0);
    }
    //printf("x : %d , y : %d \n",x*-1,y);
}

//ble hid mouse button click
void hid_button_task(char button)
{
    //ESP_LOGI("hid","hid_button_task %c click", button);
    printf("button task %c \n", button);
    if(button == 'R'){
        printf("send right click hid\n");
        for (uint8_t i = 0; i < CONFIG_BT_ACL_CONNECTIONS; i++)// right click
        {
            if (active_hid_conn_ids[i] != -1)
            {
                esp_hidd_send_mouse_value(active_hid_conn_ids[i], (1 << 1), 0, 0, 0);//111111
                esp_hidd_send_mouse_value(active_hid_conn_ids[i], 0, 0, 0, 0);
            }
        }
    }else{
        printf("send left click hid\n");
        for (uint8_t i = 0; i < CONFIG_BT_ACL_CONNECTIONS; i++) //left click
        {
            if (active_hid_conn_ids[i] != -1)
            {
                esp_hidd_send_mouse_value(active_hid_conn_ids[i], (1 << 0), 0, 0, 0);//000000
                esp_hidd_send_mouse_value(active_hid_conn_ids[i], 0, 0, 0, 0);
            }
        }
    }
...
void hid_uart_task(char character){
    ...
    case 's':
        ...
        {
            if (active_hid_conn_ids[i] != -1)
                esp_hidd_send_mouse_value(active_hid_conn_ids[i], 0, 0, MOUSE_SPEED, 0);
        }
